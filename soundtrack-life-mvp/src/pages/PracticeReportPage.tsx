...
export default function PracticeReportPage() {
  ...
  return <main className="practice-mobile report-page report-design warm-room-page">
    ...
    <section className="report-sheet coaching-report-sheet">
      ...
      </article>)}</section><div className="report-quality"><div><Activity /><span><b>{Math.round(report.dataQuality.vocalCoverage * 100)}%</b><small>有效覆盖</small></span></div><div><Gauge /><span><b>{Math.round(report.dataQuality.pitchConfidence * 100)}%</b><small>音高置信度</small></span></div><div><ShieldCheck /><span><b>{report.dataQuality.noiseFloorDb.toFixed(0)} dB</b><small>环境底噪</small></span></div></div></details>
      <div className="report-actions">{!sessionId && <button onClick={() => navigate(`/practice/${report.songId}/highlight`)}><Play />查看高光</button>}<button onClick={() => navigate(`/practice/${report.songId}/sing`)}><RefreshCw />再练这首</button></div>
      <p className="report-disclaimer"><ShieldCheck />分数只作为底层依据；小麦优先给出轻量、可练的一条反馈。</p>
    </section>
  </main>
}
