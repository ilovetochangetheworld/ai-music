...
import { ArrowLeft, BarChart3, Download, Music2, RefreshCw, Sparkles, Trash2 } from 'lucide-react'
...
export default function SingRoomRecapPage() {
  ...
  return <main className="practice-mobile recap-design warm-room-page">
    <header className="warm-page-header"><button onClick={() => navigate('/')} title="返回首页"><ArrowLeft /></button><h1>本次结算</h1><span /></header>
    ...
    <section className="recap-result-card">
      ...
      <div className="recap-design-actions single"><button onClick={() => navigate(`/practice/${songId}/report`)}><BarChart3 />查看成长报告</button></div>
      <p className="recap-tip">💡 每次练一练，进步看得见～</p>
    </section>
    <button className="recap-retry" onClick={() => navigate(`/practice/${songId}/sing`)}><RefreshCw />再练一首</button>
  </main>
}
